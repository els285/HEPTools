...
class _1D_Posterior_Plot:
    ...
    def __init__(self,posterior,limits,**kwargs):
        ...
        hist = bh.Histogram(bh.axis.Regular(200,mini, maxi))
        hist.fill(self.posterior)

        self.Primary_Hist_bh = hist/hist.sum()
        self.Primary_Hist_np = self.Primary_Hist_bh.to_numpy()        
 
        self.Region_Hists_bh = {"_3sig_left"  : self.Primary_Hist_bh[ bh.loc(mini)                     : bh.loc(self.limits["2sigma"][0])  ],
                             "_2sig_left"     : self.Primary_Hist_bh[ bh.loc(self.limits["2sigma"][0]) : bh.loc(self.limits["1sigma"][0])  ],
                             "_1sig"          : self.Primary_Hist_bh[ bh.loc(self.limits["1sigma"][0]) : bh.loc(self.limits["1sigma"][1])  ], 
                             "_2sig_right"    : self.Primary_Hist_bh[ bh.loc(self.limits["1sigma"][1]) : bh.loc(self.limits["2sigma"][1])  ],
                             "_3sig_right"    : self.Primary_Hist_bh[ bh.loc(self.limits["2sigma"][1]) : bh.loc(maxi)                      ]
        } 

        self.Region_Hists_np = {k:v.to_numpy() for k,v in self.Region_Hists_bh.items()}
    ...
    def make_multi_plot(self):
        ...
        hep.histplot(self.Region_Hists_np["_3sig_left"],   fill=True, color="red")
        hep.histplot(self.Region_Hists_np["_3sig_right"],  fill=True, color="red")
        hep.histplot(self.Region_Hists_np["_2sig_left"],   fill=True, color="yellow")
        hep.histplot(self.Region_Hists_np["_2sig_right"],  fill=True, color="yellow")
        hep.histplot(self.Region_Hists_np["_1sig"],        fill=True, color="lawngreen")

        # Black outline
        hep.histplot(self.Primary_Hist_np,fill=False,color="black",linewidth=0.4)
