...
def cos_n(Kdirection,leptonP_direction,leptonM_direction):
    ...
    Ndirection = z.cross(Kdirection)/(1-(z.dot(Kdirection))**2)*0.5

    cos_plus  = Ndirection.dot(leptonP_direction)
    cos_minus = -Ndirection.dot(leptonM_direction) 
    return cos_plus, cos_minus
...
def cos_r(Kdirection,leptonP_direction,leptonM_direction):
    ...
    z = vector.obj(x=0,y=0,z=1)
    y = z.dot(Kdirection)
    Rdirection =  y / (1-y**2)*0.5 * (z - Kdirection)

    cos_plus  = Rdirection.dot(leptonP_direction)
    cos_minus = -Rdirection.dot(leptonM_direction) 
    return cos_plus, cos_minus
